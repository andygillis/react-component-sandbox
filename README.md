...
* The `npm start` command is actually correlating to the start script in *package.json*. This command starts the `webpack-dev-server`, which in turn reads in *webpack.config.js*, does the transpiling (using Babel), packs the JavaScript, and starts a web server on port 3000. 
